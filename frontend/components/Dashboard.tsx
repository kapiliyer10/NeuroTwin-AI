...
export default function Dashboard() {
  ...
  return (
    <main className={`dashboard ${selectedPurpose}`}>
      ...
      <div className="modeRail" aria-label="Current support mode"><span className="modeIcon">{selectedPurpose === "concussion" ? "R" : "W"}</span><div><strong>{selectedPurpose === "concussion" ? "Recovery rhythm" : "Wellbeing rhythm"}</strong><small>{hasCheckIn ? (selectedPurpose === "mental_wellbeing" ? "Saved for today · return tomorrow or after a meaningful change" : "Your latest check-in is shaping today's view") : (selectedPurpose === "mental_wellbeing" ? "One intentional check-in per day is enough" : "A small check-in can make the next step clearer")}</small></div><span className="modeStep">{hasCheckIn ? "Updated" : "Ready when you are"}</span></div>
      ...
      <div className="mainGrid">
        <form className="panel checkIn" onSubmit={submitCheckIn}>
          ...
          <div className="formRow twoColumns">
            {selectedPurpose === "concussion" ? <label>Activity type<select value={form.activity_type} onChange={(event) => updateField("activity_type", event.target.value)}>{["daily", "school", "work", "screen", "walking", "exercise", "sport"].map((value) => <option key={value}>{value}</option>)}</select><small className="fieldHint">Sport activates an extra safety check. Other choices add context to your record.</small></label> : <label>Check-in context<select value={form.activity_type} onChange={(event) => updateField("activity_type", event.target.value)}>{["daily", "school", "work", "screen"].map((value) => <option key={value}>{value}</option>)}</select><small className="fieldHint">Choose what part of your day you are reflecting on. It does not change your wellbeing score.</small></label>}
            {selectedPurpose === "concussion" && <label>Activity duration<input type="number" min="0" max="1440" value={form.activity_minutes} onChange={(event) => updateField("activity_minutes", Number(event.target.value))} /><small className="fieldHint">Recorded as context for this activity; it is not used as a medical measurement.</small></label>}
          </div>
          ...
          {selectedPurpose === "mental_wellbeing" && <p className="cadenceNote">This is a snapshot, not a live score. Move the sliders to prepare a check-in; the result changes only when you save.</p>}
          <button className="primaryButton" type="submit" disabled={isLoading}>{isLoading ? "Saving check-in..." : hasCheckIn && selectedPurpose === "mental_wellbeing" ? "Save updated check-in" : "Save check-in"}</button>
        </form>
